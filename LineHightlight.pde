    ...
    try{
        ...
        while((str = read.readLine()) != null)
        {
            if(str.length() == 1) //Check to see if we are reading the first line
            {
              ...
              segs = new Segment[Integer.parseInt(str)+1];
              upperbound = new Segment("segment-0", 0, 800, 400, 0, 0, 0);
              segs[0] = upperbound;
              //totalsegs += 1;
            }
...
ArrayList<Segment> findNewSegment(Segment seg){
  float x1=-1, x2=0;
  ArrayList<Segment> list = new ArrayList<Segment>();
  Segment newseg = new Segment("new", 0, 0, 0, 0, 0, 0);
  for(float j=seg.getX1(); j<seg.getX2(); j++){
    if(checker[Math.round(j)]==true){
      if(x1==-1 || x2==0){}
        ...
        //println("000");
        //println(x1);
        newseg = new Segment("new", x1, x2, seg.getY(), 0, 0, 0);
        fillChecker(x1, x2);
        list.add(newseg);
        x1=-1;
        x2=0;
...
Segment[] GenerateLowerEnvelope(Segment[] seg){
  ...
  for(int y=0; y<=400; y++){
    for(int i=0; i<seg.length; i++){
      if(y == seg[i].getY()){
         if(i==0){
            lowerE[i] = new Segment("LE:"+i, seg[i].getX1(), seg[i].getX2(), seg[i].getY(), 0, 0, 0);
          ...
          if(seg[i].getX2() > seg[i-1].getX2() && seg[i].getX1() < seg[i-1].getX1()){
            if(checkUsed(seg[i].getX1(), seg[i-1].getX1())==0){}
              ...
              lowerE[i] = new Segment("LE:"+i, seg[i].getX1(), seg[i-1].getX1(), seg[i].getY(), 0, 0, 0);
              //println(lowerE[i]);
              //println(seg[i].getX1());
            ...
            if(checkUsed(seg[i-1].getX2(), seg[i].getX2())==0){}
              ...
              //println("checkcheck2");
              lowerE2[i] = new Segment("LE2:"+i, seg[i-1].getX2(), seg[i].getX2(), seg[i].getY(), 0, 0, 0);
             ...
             if(seg[i].getX1() < seg[i-1].getX2()){
               ...
               if(checkUsed(seg[i-1].getX2(), seg[i].getX2())==0){}
                 ...
                 lowerE[i] = new Segment("LE2:"+i, seg[i-1].getX2(), seg[i].getX2(), seg[i].getY(), 0, 0, 0);
                 fillChecker(seg[i-1].getX2(), seg[i].getX2());
               }
               else{
                 ArrayList<Segment> pass = new ArrayList<Segment>();
               ...
               lowerE2[i] = new Segment("LE2:"+i, maxX1, seg[i].getX2(), seg[i].getY(), 0, 0, 0);
             ...
             if(checkUsed(seg[i].getX1(), minX1)==0){}
               ...
               lowerE[i] = new Segment("LE2:"+i, seg[i].getX1(), minX1, seg[i].getY(), 0, 0, 0);
...
void mousePressed()
{
    ...
    {
       clicks++;
       Segment cp = new Segment("segment-0", 0, 0, 0, 0, 0, 0);
